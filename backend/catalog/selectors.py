...
from django.db.models import Avg, Case, Count, FloatField, IntegerField, Max, Min, Prefetch, Value, When
    ...
    def walk(parent_id: int | None, depth: int) -> None:
        for node in by_parent.get(parent_id, []):
            if max_depth is not None and depth > max_depth:
                continue
            rows.append({"id": node.id, "slug": category_slug_path(node, by_id), "name": node.name, "depth": depth, "label": f'{"  " * depth}{node.name}'})
...
def facet_option_counts(qs, field_name: str, *, label_field: str = "name", limit: int = 12) -> list[dict]:
    values = qs.exclude(**{f"{field_name}__isnull": True}).values(f"{field_name}_id", f"{field_name}__{label_field}").annotate(count=Count("id")).order_by("-count", f"{field_name}__{label_field}")[:limit]
    options = []
    for row in values:
...
def seller_facet_counts(qs, limit: int = 10) -> list[dict]:
    values = qs.exclude(seller__seller_store__isnull=True).values("seller_id", "seller__seller_store__name", "seller__seller_store__slug").annotate(count=Count("id")).order_by("-count", "seller__seller_store__name")[:limit]
    options = []
    for row in values:
        if not row.get("seller_id"):
            continue
        options.append({"id": row["seller_id"], "slug": row.get("seller__seller_store__slug", "") or "", "label": row.get("seller__seller_store__name", "") or "", "count": row.get("count", 0)})
    return options


def with_rating(qs):
    return qs.annotate(rating_avg=Coalesce(Avg("reviews__rating"), Value(0.0), output_field=FloatField()), rating_count=Count("reviews", distinct=True))


def ordered_products_with_related(product_ids, include_rating: bool = True):
    if not product_ids:
        return []
    order_case = Case(*[When(id=pid, then=pos) for pos, pid in enumerate(product_ids)], default=len(product_ids), output_field=IntegerField())
    base_qs = Product.objects.filter(id__in=product_ids).only(
        "id", "slug", "name", "price", "stock_qty", "min_order_qty", "lead_time_days", "pack_qty", "unit", "material", "volume_ml", "is_new", "is_promo", "brand__name", "brand__slug", "series__name", "category__name", "category__slug", "seller__seller_store__slug", "seller__seller_store__name"
    ).select_related("brand", "series", "category", "seller", "seller__seller_store").prefetch_related(
        Prefetch("images", queryset=ProductImage.objects.only("id", "product_id", "url", "alt", "ordering").order_by("ordering", "id"), to_attr="prefetched_images"),
        Prefetch("collections", queryset=Collection.objects.only("id", "name", "slug").order_by("-is_featured", "name")),
        "documents",
...
def cached_home_product_ids(limit: int = 12):
    ...
    key = f"catalog:home:product_ids:v6:{limit}:{latest_id}"
    ids = _cache_get(key)
    if ids is None:
...
def cached_home_category_ids(limit: int = 8):
    key = f"catalog:home:category_ids:v2:{limit}"
    ids = _cache_get(key)
    if ids is None:
        ids = list(Category.objects.filter(parent__isnull=True).exclude(name__startswith="HoReCa направление").order_by("id").values_list("id", flat=True)[:limit])
        _cache_set(key, ids, timeout=getattr(settings, "CACHE_TTL_HOME", 180))
    return ids


def cached_catalog_default_page_ids(page: int, page_size: int):
    key = f"catalog:default_page_ids:v4:{page}:{page_size}"
    ids = _cache_get(key)
    if ids is None:
        offset = max(0, page - 1) * page_size
        ids = list(Product.objects.order_by("-is_new", "name", "id").values_list("id", flat=True)[offset : offset + page_size])
        _cache_set(key, ids, timeout=getattr(settings, "CACHE_TTL_CATALOG_API", 120))
    return ids


def cached_catalog_default_total_count():
    key = "catalog:default_total_count:v4"
...
def resolve_category_filter(category_value: str, *, categories: list[Category] | None = None) -> Category | None:
    ...
    category_rows = categories if categories is not None else list(Category.objects.only("id", "name", "slug", "parent_id").order_by("parent_id", "name", "id"))
    ...
    if "/" in raw_value:
        ...
        cursor = None
        for part in parts:
            cursor = next((candidate for candidate in category_rows if candidate.parent_id == (cursor.id if cursor else None) and candidate.slug == part), None)
            if cursor is None:
                return None
        return cursor
