...
from legacy_shopfront_state.models import (
    BrandSubscription,
...
class SavedListOperationService:
    ...
    def delete_list(self, list_id: int) -> SavedListOperationResult:
        ...
        return SavedListOperationResult(bool(affected), "Список удалён" if affected else "Список не найден", affected_count=affected)
    ...
    def remove_item_from_list(self, list_id: int, item_id: int) -> SavedListOperationResult:
        ...
        return SavedListOperationResult(bool(affected), "Товар удалён из списка" if affected else "Товар не найден", affected_count=affected)
...
class FavoriteOperationService:
    ...
    def get_favorite_products(self, limit: int = 300) -> list[Product]:
        product_ids = list(FavoriteProduct.objects.filter(user=self.user).order_by("-created_at").values_list("product_id", flat=True)[:limit])
...
class SubscriptionOperationService:
    ...
    def toggle_subscription(self, entity: str, entity_id: int) -> SavedListOperationResult:
        model_map = {"brand": (BrandSubscription, Brand, "brand"), "category": (CategorySubscription, Category, "category")}
        ...
        if not created:
            obj.delete()
        return SavedListOperationResult(True, "Подписка обновлена", affected_count=1, meta={"entity": entity, "entity_id": entity_id, "subscribed": created})
...
class SavedSearchService:
    ...
    def save_search(self, querystring: str, name: str = "") -> SavedListOperationResult:
        ...
        saved_search = SavedSearch.objects.create(user=self.user, name=((name or "").strip() or "Мой фильтр")[:120], querystring=querystring[:512])
    ...
    def delete_search(self, search_id: int) -> SavedListOperationResult:
        ...
        return SavedListOperationResult(bool(affected), "Сохранённый поиск удалён" if affected else "Поиск не найден")
