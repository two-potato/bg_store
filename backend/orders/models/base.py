"""Core order and order-item models."""
...
class Order(TimeStampedModel):
    ...
    class PaymentMethod(models.TextChoices):
        ...
        ONLINE_CARD = "online_card", "Онлайн-оплата"
    ...
    delivery_slot = models.CharField(max_length=120, blank=True, default="")
    ...
    def recalc_totals(self, explicit_discount_amount: Decimal | None = None):
        ...
        self.subtotal = sum((item.price * item.active_qty for item in items), start=Decimal("0.00")).quantize(Decimal("0.01"))
        if explicit_discount_amount is None:
            ...
            self.discount_amount = min(self.subtotal, Decimal(str(explicit_discount_amount)).quantize(Decimal("0.01")))
