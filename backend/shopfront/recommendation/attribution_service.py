...
from .observability import observe_recommendation_event
from .policy import remember_recommendation_dismiss
...
def _ensure_state(request) -> dict:
    """Internal helper for ensure state."""
...
def _base_entry(payload: dict, *, event_name: str) -> dict:
    """Internal helper for base entry."""
    return {
        "event": event_name,
...
def remember_recommendation_feedback(request, payload: dict) -> None:
    """Remember recommendation feedback."""
    ...
    if event_name == "recommendation_impression":
        for position, item in enumerate(((payload.get("ecommerce") or {}).get("items") or []), start=1):
            ...
            entry["impression_id"] = str(item.get("impression_id") or entry.get("impression_id") or "")
        ...
        if product_id:
            ...
            entry["impression_id"] = str(payload.get("impression_id") or entry.get("impression_id") or "")
...
def recommendation_attribution_for_product(request, product_id: int) -> dict:
    """Handle recommendation attribution for product."""
    state = _ensure_state(request)
    now_ts = _now_ts()
...
def bind_cart_item_recommendation_attribution(request, *, product_id: int, attribution: dict) -> None:
    """Bind cart item recommendation attribution."""
    if not attribution:
        return
    state = _ensure_state(request)
...
def cart_item_recommendation_attribution(request, *, product_id: str | int) -> dict:
    """Handle cart item recommendation attribution."""
    state = _ensure_state(request)
...
def remove_cart_item_recommendation_attribution(request, *, product_id: str | int) -> None:
    """Remove cart item recommendation attribution."""
    state = _ensure_state(request)
...
def clear_cart_recommendation_attribution(request) -> None:
    """Clear cart recommendation attribution."""
    state = _ensure_state(request)
    state["cart_items"] = {}
    request.session[RECOMMENDATION_ATTRIBUTION_SESSION_KEY] = state
    request.session.modified = True
...
def remember_order_recommendation_attribution(request, *, order: Order, attribution: dict) -> None:
    """Remember order recommendation attribution."""
    if not attribution:
        return
    state = _ensure_state(request)
...
def order_recommendation_attribution(request, order: Order) -> dict:
    """Handle order recommendation attribution."""
    state = _ensure_state(request)
...
def record_recommendation_event(
    *,
    ...
    row = RecommendationEvent.objects.create(
        ...
        payload={
            **payload,
            "recommendation_id": str(attr.get("recommendation_id") or payload.get("recommendation_id") or ""),
            "impression_id": str(attr.get("impression_id") or payload.get("impression_id") or ""),
            "experiment_variant": experiment_variant,
            "strategy": strategy,
            "model_version": model_version,
            "engine_source": str(attr.get("engine_source") or payload.get("engine_source") or ""),
            "service_source": str(attr.get("service_source") or payload.get("service_source") or ""),
            "fallback_source": str(attr.get("fallback_source") or payload.get("fallback_source") or ""),
            "empty_reason": str(attr.get("empty_reason") or payload.get("empty_reason") or ""),
            "latency_ms": int(attr.get("latency_ms") or payload.get("latency_ms") or 0),
