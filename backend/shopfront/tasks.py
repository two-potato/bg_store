...
def refresh_recommendation_popularity(window: str = "7d", limit: int = 60):
    ...
    by_brand: dict[int, list[dict]] = defaultdict(list)
    by_seller: dict[int, list[dict]] = defaultdict(list)
    for product in products:
        ...
        if product.brand_id:
            by_brand[product.brand_id].append(row)
        if product.seller_id:
            by_seller[product.seller_id].append(row)
...
def refresh_recommendation_affinities(limit_per_product: int = 24):
    ...
    rows: list[RecommendationProductAffinity] = []
    ...
    co_view_rows = list(
        RecommendationEvent.objects.filter(event__in=["recommendation_impression", "recommendation_click"])
        .order_by("request_id", "session_key", "user_id", "created_at", "id")
        .values_list("request_id", "session_key", "user_id", "event", "product_id")
    )
    co_view_groups: dict[str, list[tuple[int, str]]] = defaultdict(list)
    for request_id, session_key, user_id, event_name, product_id in co_view_rows:
        if not product_id:
            continue
        user_key = f"user:{int(user_id)}" if user_id else ""
        key = str(request_id or "").strip() or str(session_key or "").strip() or user_key
        if not key:
            continue
        co_view_groups[key].append((int(product_id), str(event_name or "")))
    co_view_edges: Counter[tuple[int, int]] = Counter()
    co_view_clicks: Counter[tuple[int, int]] = Counter()
    for rows_for_key in co_view_groups.values():
        product_ids = sorted({product_id for product_id, _ in rows_for_key if product_id > 0})
        clicked_ids = {product_id for product_id, event_name in rows_for_key if event_name == "recommendation_click"}
        for source_product_id in product_ids:
            for target_product_id in product_ids:
                if source_product_id == target_product_id:
                    continue
                edge_key = (source_product_id, target_product_id)
                co_view_edges[edge_key] += 1
                if target_product_id in clicked_ids:
                    co_view_clicks[edge_key] += 1
    RecommendationProductAffinity.objects.filter(
        affinity_type=RecommendationProductAffinity.AffinityType.SIMILAR
    ).delete()
    similar_rows: list[RecommendationProductAffinity] = []
    grouped_view_edges: dict[int, list[tuple[int, int, int]]] = defaultdict(list)
    for (source_product_id, target_product_id), count in co_view_edges.items():
        grouped_view_edges[source_product_id].append((target_product_id, count, co_view_clicks[(source_product_id, target_product_id)]))
    for source_product_id, targets in grouped_view_edges.items():
        for target_product_id, count, click_count in sorted(targets, key=lambda item: (-(item[1] * 2 + item[2]), -item[2], item[0]))[:limit_per_product]:
            similar_rows.append(
                RecommendationProductAffinity(
                    source_product_id=source_product_id,
                    target_product_id=target_product_id,
                    affinity_type=RecommendationProductAffinity.AffinityType.SIMILAR,
                    score=Decimal(count * 2 + click_count),
                    orders_count=click_count,
                    views_count=count,
                )
            )
    RecommendationProductAffinity.objects.bulk_create(similar_rows, batch_size=200)
    total_rows = len(rows) + len(similar_rows)
    log.info("recommendation_affinities_refreshed", extra={"edges": total_rows, "copurchase_edges": len(rows), "similar_edges": len(similar_rows)})
    return total_rows
...
def refresh_recommendation_sets(limit: int = 8):
    ...
    user_ids = sorted(
        {
            int(value)
            for value in (
                list(RecommendationUserAffinity.objects.order_by().values_list("user_id", flat=True).distinct()[:500])
                + list(FavoriteProduct.objects.order_by().values_list("user_id", flat=True).distinct()[:500])
                + list(RecentlyViewedProduct.objects.order_by().values_list("user_id", flat=True).distinct()[:500])
            )
            if value
        }
    )[:500]
    for user_id in user_ids:
        ...
        recent_ids = user_affinity_seed_ids(user_ref, limit=limit)
        if recent_ids:
            _set_recommendation_set(
                kind="recently_viewed_home",
                scope_type=RecommendationSet.ScopeType.USER,
                scope_id=user_id,
                source="recent_history",
                product_ids=recent_ids[:limit],
                expires_in_sec=2 * 3600,
            )
        reorder_ids = list(
...
def train_recommendation_ml_surface(surface: str = "home", label_kind: str = "purchase", activate: bool = True):
    ...
    log.info(
        "recommendation_ml_surface_trained",
        extra={
            "surface": surface,
    ...
    return {"dataset_id": dataset.id, "model_id": model.id, "model_status": model.status}
