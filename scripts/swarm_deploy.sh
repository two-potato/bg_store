...
DESIRED_SEARCH_API_REPLICAS="${SEARCH_API_REPLICAS:-1}"
DESIRED_RECOMMENDATION_API_REPLICAS="${RECOMMENDATION_API_REPLICAS:-1}"
...
SEARCH_API_REPLICAS=0 \
RECOMMENDATION_API_REPLICAS=0 \
...
export SEARCH_API_REPLICAS="$DESIRED_SEARCH_API_REPLICAS"
export RECOMMENDATION_API_REPLICAS="$DESIRED_RECOMMENDATION_API_REPLICAS"
docker stack deploy --with-registry-auth --prune -c "$STACK_FILE" "$STACK_NAME"
